...
@Component({
	selector: 'app-create-overtime-request',
	templateUrl: './create-overtime-request.component.html',
	styleUrls: ['./create-overtime-request.component.scss']
})
...
export class CreateOvertimeRequestComponent implements OnInit {
	...
	overtimeDate: any = new Date();
	overtimeEndDate: any = new Date();

	minDate: any = new Date();
	maxDate: any = new Date();

	overtimeCategory: any;
	remarks: any;
	...
	public category: any = [
		{
			content: 'Incident Handling/Problem Fix'
		},
		{
			content: 'Preventive Maintenance (Based on Customer Request)'
		},
		{
			content: 'Project (Implementation/Change Request/Meeting at Client)'
		},
		{
			content: 'Account Support (Project Preparation, Deliverable Report Preparation, Solution Submission)'
		},
		{
			content: 'Backup Team (BAU Support)'
		},
	];
	...
	async ngOnInit() {
		...
		this.submitForm = this.fb.group({
			...
			overtimeEndDate: [new Date(), Validators.required]
		});
	...
	reqOvertime() {
		...
		if (this.startHour === undefined || this.startMinute === undefined) {
		...
		} else if (this.overtimeCategory === undefined || this.overtimeCategory === '') {
			this.toastr.error('Please enter the overtime category');
			...
			this.toastr.error('Please allow location services to get your location to submit overtime');
		} else {
			...
			const currDate = new Date(this.overtimeDate);

			this.startTime = new Date(this.overtimeDate);
			// tslint:disable-next-line: radix
			this.startTime.setHours(parseInt(this.startHour), parseInt(this.startMinute), 0, 0);
			console.log(this.startTime);

			this.endTime = new Date(this.overtimeEndDate);
			// tslint:disable-next-line: radix
			this.endTime.setHours(parseInt(this.endHour), parseInt(this.endMinute), 0, 0);
			console.log(this.endTime);

			if (currDate < nowDate) {
				this.toastr.error('Overtime Date cannot be in the past.', 'Invalid Overtime Date');
				...
				this.employeeService.requestOvertime({
					'geoTag': {
						'coordinates': {
					...
					'overtimeCategory': this.overtimeCategory.content,
					'remark': this.remarks,
					'timezone': timeZone
				})
					.subscribe(
	...
	getEmployeeOvertimeCount() {
		this.employeeService.getOvertimeCount()
			.subscribe(
				response => {
					if (response !== null) {
